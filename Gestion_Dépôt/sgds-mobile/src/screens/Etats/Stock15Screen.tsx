...
.badge{display:inline-block;background:#0E7490;color:#FFF;padding:3px 12px;border-radius:20px;font-size:12px;font-weight:800;margin-bottom:4px}
...
<div class="ph">
  <div class="ph-right">
    <div class="badge">STOCK @15°C</div><br/>
    <strong>${data.marketeur_nom}</strong>
  </div>
  <div class="ph-title">STOCK À 15°C — OUVERTURE / FERMETURE</div>
  <div class="ph-sub">Système de Gestion des Dépôts Pétroliers</div>
  <div class="ph-sub" style="margin-top:8px">${data.periode_nom}</div>
</div>
<div class="kpi">
  <div class="k"><div class="k-lbl">Stock Ouverture</div><div class="k-val">${fmtN(data.total_ouverture)}</div><div style="font-size:9px;color:#6B7589">litres @15°C</div></div>
  <div class="k" style="background:#D8F3E2"><div class="k-lbl">Entrées</div><div class="k-val" style="color:#1F9D55">${fmtN(data.total_entrees)}</div><div style="font-size:9px;color:#6B7589">litres @15°C</div></div>
  <div class="k" style="background:#FBE0E0"><div class="k-lbl">Sorties</div><div class="k-val" style="color:#D63B3B">${fmtN(data.total_sorties)}</div><div style="font-size:9px;color:#6B7589">litres @15°C</div></div>
  <div class="k"><div class="k-lbl">Stock Fermeture</div><div class="k-val" style="color:#0E2A47">${fmtN(data.total_fermeture)}</div><div style="font-size:9px;color:#6B7589">litres @15°C</div></div>
</div>
...
export function Stock15Screen() {
  ...
  const load = useCallback(async (periodeId?: number, isRefresh = false) => {
    ...
    setError(null);
    try {
      const r = await etatsApi.stock15(periodeId);
      setData(r.data);
    } catch (e: any) {
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: 'Stock à 15°C' });
      } else {
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        <View style={styles.heroContent}>
          ...
          <Text style={styles.heroTitle}>Stock à 15°C</Text>
      ...
      <View style={styles.filterRow}>
        <TouchableOpacity style={styles.perBtn} onPress={() => setShowModal(true)}>
          <Ionicons name="calendar-outline" size={14} color="#0E7490" />
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <Ionicons name="thermometer-outline" size={48} color={Colors.silver} />
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}
          ...
          <View style={styles.kpiRow}>
            <View style={[styles.kpiCard, { backgroundColor: '#CFFAFE' }]}>
              ...
              <Text style={styles.kpiUnit}>L @15°C</Text>
            </View>
  ...
  return (
    <View style={[styles.prodRow, { backgroundColor: bg }]}>
      ...
      <View style={styles.prodCols}>
        <ColVal label="Ouverture" value={ligne.stock_ouverture} color="#0E7490" />
        <ColVal label="Entrées"   value={ligne.entrees}         color={Colors.entree} />
        <ColVal label="Sorties"   value={ligne.sorties}         color={Colors.sortie} />
        <ColVal label="Fermeture" value={ligne.stock_fermeture} color="#0E7490" bold />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  perBtn: {
    ...
    gap: 6, backgroundColor: '#CFFAFE',
    borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8,
  },
  perBtnText: { flex: 1, fontSize: 13, fontWeight: '600', color: '#0E7490' },
  clearBtn:   { padding: 4 },
  ...
  fermCard: {
    backgroundColor: '#0E7490', borderRadius: 14,
  ...
  sigleBadge: {
    backgroundColor: '#0E7490', borderRadius: 6,
  ...
  totalRow: {
    backgroundColor: '#0E7490', borderRadius: 12,
  ...
  perItemActive: { backgroundColor: '#0E7490' },
